...
import {
  Home, BarChart2, AlertCircle, Tag, Zap, Trophy, CalendarDays, DollarSign,
  TrendingUp, TrendingDown, Target, Star, ThumbsUp, ThumbsDown,
  Radio, Minus, ShieldCheck, Award, Check, X, ChevronRight, Lightbulb, Music, Sparkles, ArrowUpRight, Search,
} from 'lucide-react';
...
const problemMatrix = [
  { rank: 1, problem: '체크아웃 청소 의무 과다', urgency: 9, wtp: 9, complaintCount: 6, growthRate: '↑ 급증', flags: ['🔴', '🚀'], quote: '"쓰레기 안버리면 4만원 추가된다. 분리수거 해라." — 수빈(초고층)' },
  { rank: 2, problem: '체크인·아웃 시간 경직성', urgency: 8, wtp: 9, complaintCount: 8, growthRate: '↑ 급증', flags: ['🔴', '🚀'], quote: '"체크아웃이 늦으니 더 여유로워서 좋았습니다" — 칭찬이 반복 = 11시가 기본의 불만' },
  { rank: 3, problem: '방음 불량', urgency: 8, wtp: 9, complaintCount: 4, growthRate: '↑ 증가', flags: ['🔴', '🚀'], quote: '"다른방에서 경찰이 오고 많이 시끄러웠지만 잘 잤습니다~" — 지현(자쿠지)' },
  { rank: 4, problem: '시설 노후화·파손', urgency: 7, wtp: 8, complaintCount: 4, growthRate: '→ 유지', flags: ['🔴'], quote: '"화장실 미닫이문이 들려있어서 수리가 시급해보여요!" — 미정(초고층)' },
  { rank: 5, problem: '사진·실제 불일치', urgency: 8, wtp: 8, complaintCount: 3, growthRate: '→ 유지', flags: ['🔴'], quote: '"사진과 정말 똑같았어요"가 칭찬으로 반복 등장 → 불일치가 업계 기본값임을 방증' },
  { rank: 6, problem: '난방·냉방·습도 불균등', urgency: 9, wtp: 8, complaintCount: 3, growthRate: '→ 유지', flags: ['🔴'], quote: '"2층은 건조하고 1층은 난방에선 좀 아쉬웠습니다" — 복층 숙소 공통 문제' },
  { rank: 7, problem: '호스트 응답 지연', urgency: 8, wtp: 7, complaintCount: 2, growthRate: '→ 유지', flags: ['🔴'], quote: '"응답률 100%, 1시간 이내 응답"이 칭찬 반복 → 나머지는 반대 상황' },
  { rank: 8, problem: '대중교통 접근성', urgency: 7, wtp: 5, complaintCount: 3, growthRate: '↓ 감소', flags: ['🔴'], quote: '"도보로 여행하는 사람에게는 역에서 접근성이 안 좋다" — 현주(매일바다)' },
  { rank: 9, problem: 'OTT·스트리밍 기대 상승', urgency: 5, wtp: 7, complaintCount: 1, growthRate: '↑ 급증', flags: ['🚀'], quote: 'Netflix 포함 여부가 선택 기준으로 모든 리스팅에 명시 → 이미 기본 기대치' },
  { rank: 10, problem: '블라인드·프라이버시 부재', urgency: 6, wtp: 7, complaintCount: 2, growthRate: '→ 유지', flags: ['🔴'], quote: '"욕조&베란다에 블라인드가 없어서 벽에 붙어서 샤워했네요" — 지현(자쿠지)' },
];

const offers = [
  {
    title: 'No-Chore Checkout', headline: '청소는 우리가, 마지막 아침은 당신이', urgency: 9, wtp: 9,
  ...
  {
    title: 'Silent Room 인증', headline: 'dB 수치로 증명하는 방음 보장', urgency: 8, wtp: 9,
    description: '입실 전 소음 측정값(dB) 공개. 35dB 기준치 초과 시 즉시 환불.',
    tiers: [
  ...
  {
    title: 'FlexPass', headline: '체크인 12~22시 · 체크아웃 8~15시 자유 선택', urgency: 8, wtp: 9,
  ...
  {
    title: 'Climate Ready', headline: '도착 1시간 전, 완벽한 온도로 세팅됩니다', urgency: 9, wtp: 8,
    description: '입실 전 온도·가습 사전 세팅. 복층 층별 개별 관리.',
    tiers: [
  ...
  {
    title: 'Photo Match Guarantee', headline: '사진이 다르면 30분 내 전액 환불', urgency: 8, wtp: 8,
...
const marketAnalysis = {
  tam: { label: 'TAM — 전체 시장', value: '₩5조+', description: '국내 단기 숙박 전체 (에어비앤비 + 야놀자 + 여기어때 + 호텔 포함)', note: '한국 숙박업 연간 매출 기준 추정' },
  sam: { label: 'SAM — 유효 시장', value: '₩500억', description: '인천/영종도 에어비앤비·단기 렌탈 커플 오션뷰 숙박 시장', note: '영종도 숙소 ~500개 × 평균 ₩12만/박 × 연간 가동 600박 추정' },
  som: { label: 'SOM — 획득 가능', value: '₩50억', description: '커플 전용 큐레이션 채널로 3년 내 현실적으로 공략 가능한 규모', note: '상위 20개 숙소 × 연간 예약 500건 × 평균 ₩15만 기준' },
  trends: [
    { rank: 1, title: '2030 커플 호캉스 수요 급증', description: '서울 25~35세 직장인 커플의 "SNS 인증 단기 여행" 문화가 자리잡음. 영종도는 공항 접근성 덕분에 서울에서 당일~1박 코스로 급부상.', signal: '구글 트렌드 "영종도 커플 숙소" 검색량 전년 대비 +40% 추정 · 에어비앤비 상위 숙소 후기 수 지속 증가', color: 'blue' },
    { rank: 2, title: '레이트 체크아웃 = 강력한 선택 기준', description: '오후 2시 체크아웃이 숙소 제목에 키워드로 등장할 만큼 선택 기준화. 11시 체크아웃은 이제 불만 포인트.', signal: '"늦은오후2시체크아웃" 제목 숙소의 리뷰 153개 · 재방문율 단골 언급 최다 — 직접 수요 확인', color: 'purple' },
    { rank: 3, title: '공항 근접 힐링 스테이 부상', description: '해외여행 전/후 1박 수요, 공항 환경 특수(입출국 전 릴렉스). 차량 없이 인천공항에서 15분 내 오션뷰 숙소 접근 가능.', signal: '매일바다 Islet 후기: "공항 근처 숙소로 더할 나위 없었습니다" 다수 · 셔틀 서비스 숙소 예약 우선순위 언급', color: 'teal' },
    { rank: 4, title: '감성 인테리어 + OTT = 기본 스펙화', description: 'Netflix 없으면 예약 안 함. 일본감성·한옥감성·무드조명은 이제 "있으면 좋은 것"이 아닌 "없으면 감점".', signal: '전체 5개 숙소 100% Netflix 제공 명시 · "조명·향기·음악" 칭찬 반복 등장 — 3요소 패키지 수요', color: 'orange' },
    { rank: 5, title: '기념일 특화 숙박 수요 부상', description: '생일·연애기념일 특별 데코 요청 댓글·메시지 증가. 호텔 기념일 패키지 대비 가격 경쟁력 있는 에어비앤비 기념일 오퍼 부재.', signal: '한옥Stay 자쿠지 후기: 기념일 관련 언급 다수 · "Photo Match + 장식 세팅" 요청 댓글 등장', color: 'pink' },
  ],
  opportunities: [
    { rank: 1, title: '조식 포함 오션뷰 숙소', description: '영종도 에어비앤비 전체를 통틀어 조식 제공 숙소 극히 드묾.', evidence: '검색 결과 상위 50개 중 조식 포함 숙소 2개 미만', potential: '예약 단가 +₩20,000~30,000 가능', color: 'emerald' },
    { rank: 2, title: '기념일 데코레이션 패키지', description: '케이크+꽃+조명+풍선 설치 서비스를 제공하는 숙소 부재.', evidence: '에어비앤비 검색 "기념일 영종도" 결과에서 데코 포함 숙소 0개', potential: '패키지 당 +₩45,000 · 기념일 예약 비율 30% 달성 시 월 +₩200만+', color: 'emerald' },
    { rank: 3, title: '얼리체크인 + 레이트체크아웃 번들', description: '"오후3시 체크인 + 오후2시 체크아웃" 번들 미존재.', evidence: '모든 숙소가 시간당 개별 과금 — 번들 가격 인식 차이로 전환율 개선 여지', potential: '번들가 ₩30,000 vs 개별 ₩40,000 → 예약 전환율 +20% 추정', color: 'emerald' },
    { rank: 4, title: '반려동물 동반 오션뷰 커플 숙소', description: '펫 프렌들리 + 오션뷰 + 커플 특화 조건을 동시에 충족하는 숙소 공급 극히 부족.', evidence: '영종도시티오션뷰만 반려동물 허용 — 수요 다수인데 선택지 1개', potential: '펫 커플 세그먼트 가격 프리미엄 +15~20%', color: 'emerald' },
    { rank: 5, title: '공항 셔틀 × 오션뷰 프리미엄 번들', description: '오션뷰 + 커플 감성 + 공항 셔틀을 패키지화한 프리미엄 오퍼 미존재.', evidence: '레인보우 감성숙소: 현금 결제 셔틀 운영 중이나 마케팅 미활용', potential: '공항 이용자 타깃 예약 전환 강화', color: 'emerald' },
  ],
  moneyFlows: [
...
const topRooms = [
  { rank: 1, name: '매일바다 Islet', url: 'https://www.airbnb.co.kr/rooms/1112639063232772111', image: 'https://a0.muscache.com/im/pictures/hosting/Hosting-1112639063232772111/original/a0a92697-a1fc-46db-aef4-eb5f02fe0f65.jpeg', rating: 4.97, reviewCount: 243, isSuperhost: true, priceWeekday: '₩10~13만', priceWeekend: '₩15~18만', titleKeywords: ['봄바다', '커플데이트', '섬뷰', '침대앞바다', '공항근처'], instantBook: true, minNights: 1, amenities: ['넷플릭스', '유튜브', '무료주차', '전기차충전', '에어컨'], hostResponseRate: '100%', hostResponseTime: '1시간 이내', lastReviewDate: '2026-04-08', thumbnailStrategy: '침대 위에서 찍은 바다뷰 와이드샷 — 침대+바다 동시 프레임', rankingStrength: '슈퍼호스트 + 후기 243개(최다) + 4.97점 + 즉시예약 + 계절별 키워드 업데이트' },
  { rank: 2, name: '영종도 늦은오후2시체크아웃 · 테라스 시티오션뷰', url: 'https://www.airbnb.co.kr/rooms/929613935113182910', image: 'https://a0.muscache.com/im/pictures/miso/Hosting-929613935113182910/original/933cb062-6a29-440f-b2e7-6d27f2c06ac6.jpeg', rating: 4.96, reviewCount: 153, isSuperhost: true, priceWeekday: '₩8~11만', priceWeekend: '₩12~15만', titleKeywords: ['늦은오후2시체크아웃', '테라스', '시티오션뷰', '인테리어'], instantBook: true, minNights: 1, amenities: ['넷플릭스', '구글TV', '세탁기', '테라스', '보드게임'], hostResponseRate: '100%', hostResponseTime: '1시간 이내', lastReviewDate: '2026-03-28', thumbnailStrategy: '조명 켜진 아늑한 내부 + 창밖 시티뷰 — 감성 포근함 강조', rankingStrength: '"늦은오후2시체크아웃" 키워드를 제목에 직접 삽입 → 장기 체류 희망 고객 직접 검색 유입' },
  { rank: 3, name: '풀오션뷰 한옥Stay 자쿠지 스위트룸', url: 'https://www.airbnb.co.kr/rooms/1401799113294551700', image: 'https://a0.muscache.com/im/pictures/hosting/Hosting-1401799113294551700/original/88c6e443-cda5-4d5d-b8f5-2b620d37b487.jpeg', rating: 4.96, reviewCount: 187, isSuperhost: true, priceWeekday: '₩13~17만', priceWeekend: '₩18~25만', titleKeywords: ['자쿠지', '한옥감성', '풀오션뷰', '노을전망', '인천공항15분'], instantBook: true, minNights: 1, amenities: ['자쿠지', '조식뷔페', '넷플릭스', '무료주차', '에어컨'], hostResponseRate: '100%', hostResponseTime: '1시간 이내', lastReviewDate: '2026-04-05', thumbnailStrategy: '한옥 소품 + 창밖 인천대교 노을 — 문화+자연 2가지 동시 노출', rankingStrength: '자쿠지(희소 어메니티) + 조식 제공 + 상위 5% 슈퍼호스트 조합 → 고가 세그먼트 리드' },
  { rank: 4, name: '영종도시티오션뷰 · 14시체크아웃', url: 'https://www.airbnb.co.kr/rooms/41319829', image: 'https://a0.muscache.com/im/pictures/hosting/Hosting-41319829/original/5baf9d2b-5252-493e-afe8-1895c5e23fe5.jpeg', rating: 4.93, reviewCount: 171, isSuperhost: true, priceWeekday: '₩8~11만', priceWeekend: '₩12~16만', titleKeywords: ['14시체크아웃', '시티오션뷰', '반려동물허용', '구읍뱃터'], instantBook: true, minNights: 1, amenities: ['넷플릭스', '반려동물허용', '에어컨', '무료주차', '세탁기'], hostResponseRate: '100%', hostResponseTime: '1시간 이내', lastReviewDate: '2026-04-10', thumbnailStrategy: '창밖 시티오션뷰 와이드샷 — 도심+바다 조합 시각적 임팩트', rankingStrength: '"14시체크아웃" 키워드 + 반려동물 허용(니치 필터) + 6년 호스팅 경력으로 신뢰 축적' },
  { rank: 5, name: '풀오션뷰 일본감성 · 13시체크아웃 레인보우하우스', url: 'https://www.airbnb.co.kr/rooms/1186004573241827717', image: 'https://a0.muscache.com/im/pictures/miso/Hosting-1186004573241827717/original/91477865-6cb7-41f1-aa58-677466b6179e.jpeg', rating: 4.95, reviewCount: 60, isSuperhost: true, priceWeekday: '₩9~12만', priceWeekend: '₩13~17만', titleKeywords: ['풀오션뷰', '13시체크아웃', '일본감성', '테라스노을뷰'], instantBook: true, minNights: 1, amenities: ['넷플릭스', '다도세트', '에어컨', '무료주차', '테라스'], hostResponseRate: '100%', hostResponseTime: '1시간 이내', lastReviewDate: '2026-03-15', thumbnailStrategy: '일본풍 소품 + 테라스 노을 바다 — 이국적 감성 강조', rankingStrength: '일본감성(차별화 테마) + 13시체크아웃 키워드 + 노을뷰 썸네일 조합' },
];
...
const AirbnbResearch: React.FC = () => {
  ...
  const tabItems: { key: TabKey; icon: React.ReactNode; label: string }[] = [
    ...
    { key: 'problems', icon: <AlertCircle size={13} />, label: '문제 분석' },
    { key: 'offers',   icon: <Sparkles size={13} />, label: '고전환 오퍼' },
    { key: 'viral',    icon: <Zap size={13} />, label: '바이럴 전략' },
    { key: 'toprooms', icon: <Trophy size={13} />, label: '인기룸 분석' },
    { key: 'events',   icon: <CalendarDays size={13} />, label: '이벤트 캘린더' },
      ...
      <header className="relative h-[80vh] flex items-center justify-center overflow-hidden bg-black">
        ...
        <div className="relative z-10 text-center px-4">
          ...
          <div className="flex justify-center gap-6 mt-8 flex-wrap">
            {[['5', '숙소 스크래핑'], ['792', '리뷰 수집'], ['4.944', '평균 평점'], ['₩500억', 'SAM 시장']].map(([v, l], i, arr) => (
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as TabKey)}>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 py-16">
          ...
          <TabsContent value="listings">
            <h2 className="text-4xl sm:text-5xl font-serif mb-2 border-b pb-4 flex items-center gap-3"><Home size={36} strokeWidth={1.5} />영종도 커플 오션스테이</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {listings.map((item, i) => (
                <Card
                  key={i}
                  className="group cursor-zoom-in hover:shadow-xl transition-shadow duration-300 overflow-hidden ring-0 border border-slate-200"
                ...
                >
                  ...
                  <CardHeader className="pb-1">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-amber-500 font-bold text-sm flex items-center gap-1"><Star size={12} fill="currentColor" />{item.rating}</span>
                  ...
                  <CardContent className="pt-0 space-y-3">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {item.tags.map((tag, j) => (
                        <Badge key={j} variant="outline" className="text-blue-700 border-blue-200 bg-blue-50 text-xs">{tag}</Badge>
                      ))}
                    </div>
                    <div className="text-xs text-slate-500 space-y-1 border-t border-slate-100 pt-3">
                      <div className="flex items-center gap-1"><ThumbsUp size={11} className="text-emerald-500" /> {item.topPraise}</div>
                      <div className="flex items-center gap-1"><ThumbsDown size={11} className="text-red-400" /> {item.topComplaint}</div>
                    </div>
                  </CardContent>
                </Card>
              ))}
          ...
          <TabsContent value="market">
            <h2 className="text-4xl sm:text-5xl font-serif mb-2 border-b pb-4 flex items-center gap-3"><BarChart2 size={36} strokeWidth={1.5} />시장 분석</h2>
            <p className="text-slate-500 text-sm tracking-widest uppercase mb-10">TAM · SAM · SOM · 핵심 트렌드 5개 · 공백 기회 5개 · 돈의 흐름</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
              {[
                ...
                <div key={i} className={`rounded-2xl p-8 ${item.bg}`}>
                  ...
                  <p className={`text-4xl font-bold mb-3 ${item.text}`}>{item.value}</p>
            ...
            <div className="mb-16">
              <h3 className="text-2xl font-semibold mb-6 flex items-center gap-2"><TrendingUp size={20} />수요를 만드는 핵심 트렌드</h3>
                ...
                {marketAnalysis.trends.map((item, i) => {
                  ...
                  return (
                    <div key={i} className={`flex gap-4 p-5 rounded-xl border-l-4 ${c.bg} ${c.border}`}>
                      <span className="text-2xl font-bold text-slate-300 w-8 flex-shrink-0 leading-none mt-1">{String(i + 1).padStart(2, '0')}</span>
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <h4 className="font-semibold text-lg">{item.title}</h4>
                          <Badge className={`text-xs ${c.badge} border-0`}>트렌드</Badge>
                        </div>
                        <p className="text-slate-600 text-sm leading-relaxed mb-2">{item.description}</p>
                        <p className="text-xs text-slate-400 italic flex items-center gap-1"><Radio size={11} />{item.signal}</p>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="mb-16">
              <h3 className="text-2xl font-semibold mb-6 flex items-center gap-2"><Target size={20} />아직 덜 공략된 기회</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {marketAnalysis.opportunities.map((item, i) => (
                  <Card key={i} className="border border-emerald-200 bg-emerald-50 ring-0 hover:shadow-md transition-shadow">
                    <CardHeader className="pb-1">
                      <div className="flex items-start justify-between">
                        <span className="text-3xl font-bold text-emerald-200 leading-none">#{item.rank}</span>
                        <Badge variant="outline" className="text-emerald-700 border-emerald-300 bg-emerald-100 text-xs">공백 기회</Badge>
                      </div>
                      <CardTitle className="text-lg">{item.title}</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-1 text-xs">
                      <p className="text-slate-600 text-sm leading-relaxed">{item.description}</p>
                      <p className="text-slate-400 flex items-center gap-1"><Search size={11} />근거: <span className="text-slate-600">{item.evidence}</span></p>
                      <p className="text-emerald-600 font-medium flex items-center gap-1"><ArrowUpRight size={11} />잠재: {item.potential}</p>
                    </CardContent>
                  </Card>
                ))}
              </div>
            ...
            <div>
              <h3 className="text-2xl font-semibold mb-6 flex items-center gap-2"><DollarSign size={20} />이미 돈이 흐르는 영역</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {marketAnalysis.moneyFlows.map((item, i) => (
                  <Card key={i} className="border border-slate-200 ring-0">
                    <CardContent className="flex gap-4 items-start pt-4">
                      <div className="flex-shrink-0 text-slate-500">{item.trend === 'up' ? <TrendingUp size={20} className="text-emerald-500" /> : item.trend === 'stable' ? <Minus size={20} /> : <TrendingDown size={20} className="text-red-400" />}</div>
                      <div className="flex-1 min-w-0">
                        ...
                        <Badge variant="outline" className="text-slate-700 text-xs">{item.size}</Badge>
                      </div>
                    </CardContent>
                  </Card>
                ))}
          ...
          <TabsContent value="problems">
            <h2 className="text-4xl sm:text-5xl font-serif mb-2 border-b pb-4 flex items-center gap-3"><AlertCircle size={36} strokeWidth={1.5} />숙소 문제 매트릭스</h2>
            <p className="text-slate-500 text-sm tracking-widest uppercase mb-10">WTP 기준 정렬 · 실제 불평 횟수 · 성장 속도</p>
            <div className="space-y-3">
              {problemMatrix.map((item) => {
                const isActualComplaint = item.flags.includes('🔴');
                const isFastGrowing = item.flags.includes('🚀');
                const cardBg = isActualComplaint && isFastGrowing ? 'bg-red-50 border-red-200'
                  : isActualComplaint ? 'bg-orange-50 border-orange-100'
                  : isFastGrowing ? 'bg-amber-50 border-amber-100'
                ...
                const growthColor = item.growthRate.includes('급증') ? 'text-red-600 font-bold'
                  ...
                  : 'text-slate-500';
                return (
                  <Card key={item.rank} className={`ring-0 border ${cardBg}`}>
                    <CardContent className="flex items-start gap-4 pt-4">
                      <span className="text-2xl font-bold text-slate-300 w-8 flex-shrink-0">#{item.rank}</span>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <h4 className="font-semibold text-base">{item.problem}</h4>
                          {item.flags.map((f, j) =>
                            f === '🔴' ? <AlertCircle key={j} size={14} className="text-red-500" /> :
                            f === '🚀' ? <Zap key={j} size={14} className="text-amber-500" /> :
                            <span key={j}>{f}</span>
                          )}
                        </div>
                        <p className="text-slate-500 text-sm italic mb-3 leading-relaxed">{item.quote}</p>
                        <div className="flex flex-wrap gap-3 items-center">
                          <Badge variant="outline" className="text-red-600 border-red-200 text-xs">긴급도 {item.urgency}/10</Badge>
                          <div className="flex items-center gap-1.5">
                            <span className="text-slate-400 text-xs">WTP</span>
                            <div className="flex gap-0.5">
                              {Array.from({ length: 10 }, (_, k) => (
                                <div key={k} className={`w-2.5 h-2.5 rounded-sm ${k < item.wtp ? item.wtp >= 8 ? 'bg-emerald-500' : 'bg-emerald-300' : 'bg-slate-200'}`} />
                              ))}
                            </div>
                            <span className="font-bold text-emerald-600 text-xs">{item.wtp}/10</span>
                          </div>
                          {isActualComplaint && (
                            <Badge variant="secondary" className="text-xs bg-red-100 text-red-700 inline-flex items-center gap-1"><AlertCircle size={10} />{item.complaintCount}회 직접 불평</Badge>
                          )}
                          <span className={`text-xs font-medium ${growthColor}`}>{item.growthRate}</span>
                        </div>
                      </div>
                      <div className="flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-sm" style={{ background: `hsl(${Math.max(0, 120 - (item.urgency + item.wtp) * 5)}, 70%, 45%)` }}>
          ...
          <TabsContent value="offers">
            <h2 className="text-4xl sm:text-5xl font-serif mb-2 border-b pb-4 flex items-center gap-3"><Sparkles size={36} strokeWidth={1.5} />고전환 오퍼 설계</h2>
            <p className="text-slate-500 text-sm tracking-widest uppercase mb-10">Top 5 문제 · 이상적 고객 정의 · 보장 포함</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {offers.map((offer, i) => (
                <Card key={i} className="overflow-hidden ring-0 border border-slate-200 hover:shadow-lg transition-shadow">
                  ...
                  <CardContent className="pt-4 space-y-4">
                    ...
                    <div className="bg-emerald-50 rounded-lg p-3">
                      <p className="text-xs text-emerald-700 font-medium leading-relaxed flex items-start gap-1"><ShieldCheck size={12} className="flex-shrink-0 mt-0.5" />{offer.guarantee}</p>
                    </div>
                    <div className="flex flex-wrap gap-1">
          ...
          <TabsContent value="viral">
            <h2 className="text-4xl sm:text-5xl font-serif mb-2 border-b pb-4 flex items-center gap-3"><Zap size={36} strokeWidth={1.5} />바이럴 콘텐츠 전략</h2>
            <p className="text-slate-500 text-sm tracking-widest uppercase mb-10">고전환 훅 20개 · 감정 트리거 × 6 · SPICE 구조</p>
            <div className="columns-1 md:columns-2 gap-5 mb-16">
              {viralHooks.map((hook, i) => (
                <Card key={i} className={`break-inside-avoid mb-4 ring-0 border ${hookColorMap[hook.color] ?? hookColorMap.blue}`}>
                  <CardContent className="pt-4">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-xs font-bold opacity-50 uppercase tracking-widest">#{String(i + 1).padStart(2, '0')}</span>
                      <Badge variant="secondary" className="text-xs opacity-70">{hook.emotion}</Badge>
                    </div>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <Card className="ring-0 border border-slate-200">
                <CardHeader><CardTitle>감정 트리거 맵</CardTitle></CardHeader>
              ...
              <Card className="ring-0 border border-slate-200">
                <CardHeader><CardTitle>플랫폼별 전략</CardTitle></CardHeader>
                <CardContent className="space-y-3 text-sm">
                  {[
          ...
          {/* ── 인기룸 분석 ── */}
          <TabsContent value="toprooms">
            <h2 className="text-4xl sm:text-5xl font-serif mb-2 border-b pb-4 flex items-center gap-3"><Trophy size={36} strokeWidth={1.5} />Airbnb 인기룸 분석</h2>
            <p className="text-slate-500 text-sm tracking-widest uppercase mb-10">상위 노출 5개 룸 · 노출 공식 역산 · 공통 패턴</p>
            <div className="space-y-6 mb-16">
              {topRooms.map((room) => (
                <Card
                  key={room.rank}
                  className="group overflow-hidden border border-slate-200 ring-0 hover:shadow-lg transition-shadow cursor-zoom-in"
                  ...
                  <div className="flex flex-col md:flex-row">
                    <div className="md:w-64 flex-shrink-0 overflow-hidden h-48 md:h-auto relative">
                      ...
                      <div className="absolute top-3 left-3">
                        <Badge className="bg-black/70 text-white text-xs border-0">검색 {room.rank}위</Badge>
                      </div>
                    </div>
                      ...
                      <div className="flex items-start justify-between mb-3 gap-4 flex-wrap">
                        <div>
                          ...
                          <div className="flex items-center gap-2 text-sm flex-wrap">
                            <span className="text-amber-500 font-bold flex items-center gap-1"><Star size={12} fill="currentColor" />{room.rating}</span>
                            <span className="text-slate-400">{room.reviewCount}개 후기</span>
                            {room.isSuperhost && <Badge variant="secondary" className="text-xs bg-yellow-100 text-yellow-700 inline-flex items-center gap-1"><Award size={10} />슈퍼호스트</Badge>}
                            {room.instantBook && <Badge variant="secondary" className="text-xs bg-green-100 text-green-700 inline-flex items-center gap-1"><Zap size={10} />즉시예약</Badge>}
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {room.amenities.map((a, j) => (
                              <Badge key={j} variant="outline" className="text-blue-700 border-blue-200 bg-blue-50 text-xs">{a}</Badge>
                            ))}
                          </div>
                        </div>
                        <div>
                          <p className="text-xs text-slate-400 uppercase tracking-wider mb-1.5">썸네일 전략</p>
                          <p className="text-slate-600 leading-relaxed text-xs">{room.thumbnailStrategy}</p>
                        </div>
                        <div>
                          <p className="text-xs text-red-400 uppercase tracking-wider mb-1.5">노출 강점 ★</p>
                          <p className="text-slate-800 font-medium leading-relaxed text-xs">{room.rankingStrength}</p>
                        </div>
                      </div>
                    </CardContent>
                  </div>
            ...
            <div className="mb-12">
              <h3 className="text-2xl font-semibold mb-6">상위 노출 공통 패턴</h3>
                        ...
                        <td className="py-4 px-4 text-center">
                          <div className="flex items-center justify-center gap-1">
                            {Array.from({ length: p.total }, (_, k) => (
                              <div key={k} className={`w-4 h-4 rounded-full ${k < p.count ? 'bg-emerald-500' : 'bg-slate-200'}`} />
                            ))}
            ...
            <div className="bg-slate-900 text-white rounded-2xl p-8">
              <h3 className="font-semibold text-2xl mb-6">우리 룸에 즉시 적용 가능한 것</h3>
              <div className="space-y-4">
                {topRoomsActions.map((action, i) => (
                  <div key={i} className="flex gap-4 items-start">
                    <span className="w-8 h-8 rounded-full bg-emerald-500 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">{i + 1}</span>
                    <p className="text-slate-300 leading-relaxed pt-1 text-sm">{action}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Bottom Summary (listings~toprooms 탭에서만 표시) */}
          {!['events', 'pricing'].includes(activeTab) && (
            <section className="mt-24 bg-slate-50 p-8 sm:p-10 rounded-3xl">
              <div className="grid md:grid-cols-2 gap-12">
                <div>
                  <h4 className="text-2xl font-semibold mb-6 underline underline-offset-4">핵심 기회 키워드</h4>
                  <div className="space-y-3">
                    {opportunityKeywords.map((kw, i) => (
                      <div key={i} className="flex gap-3 items-start">
                        <Badge className="text-xs whitespace-nowrap mt-0.5">{kw.label}</Badge>
                ...
                <div>
                  <h4 className="text-2xl font-semibold mb-6 underline underline-offset-4">이번 주 액션 아이템</h4>
          ...
          <TabsContent value="events">
            <h2 className="text-4xl sm:text-5xl font-serif mb-2 border-b pb-4 flex items-center gap-3"><CalendarDays size={36} strokeWidth={1.5} />영종도 이벤트 캘린더</h2>
            <p className="text-slate-500 text-sm tracking-widest uppercase mb-10">2026 수요 예측 캘린더 · 인스파이어 아레나 · 인천투어 · 최종 업데이트 2026-04-12</p>
            <div className="flex flex-wrap gap-2 mb-10">
              {[
                { stars: '★★★★★', label: '초성수기', className: 'bg-red-100 text-red-700 border-red-200' },
                { stars: '★★★★', label: '성수기', className: 'bg-orange-100 text-orange-700 border-orange-200' },
                { stars: '★★★', label: '준성수기', className: 'bg-yellow-100 text-yellow-700 border-yellow-200' },
                { stars: '★★', label: '보통', className: 'bg-blue-100 text-blue-700 border-blue-200' },
                { stars: '★', label: '미미', className: 'bg-slate-100 text-slate-600 border-slate-200' },
              ].map(({ stars, label, className }) => (
                <Badge key={label} variant="outline" className={`text-xs px-3 py-1 ${className}`}>{stars} {label}</Badge>
              ))}
            </div>
            {[
              {
                month: '4월', strategy: 'T1 HOME GROUND (04.24~26) e스포츠 팬 수요 강함. 해당 주말 최소 1박 + 가격 15~20% 인상 즉시 적용.',
                events: [
              ...
              {
                month: '5월', strategy: '이벤트 공백기. 어린이날(05.05) D-14(04.21) 알림 설정 후 ₩108,000 인상 준비.',
                events: [
              ...
              {
                month: '7~8월 (성수기)', strategy: '연중 최성수기. 을왕리 해수욕장 개장일(7월 초) 확인 즉시 전 기간 성수기 요금 적용.',
                events: [
              ...
              {
                month: '10월', strategy: 'EDC 코리아는 개천절 연휴(10.03~04) 대형 EDM 페스티벌. 티켓 판매 시작 즉시 가격 대폭 인상 + 최소 2박 조건 강력 권고.',
                events: [
              ...
              <div key={month} className="mb-12">
                <h3 className="text-2xl font-semibold mb-4">{month}</h3>
                <div className="overflow-x-auto mb-3">
                  <table className="w-full text-sm border-collapse">
                    ...
                    <tbody>
                      {events.map((ev, i) => {
                        const demandColor = ev.demand === '★★★★★' ? 'text-red-600 font-bold' : ev.demand === '★★★★' ? 'text-orange-500 font-bold' : ev.demand === '★★★' ? 'text-yellow-600 font-semibold' : 'text-slate-400';
                        return (
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 text-sm text-amber-800 flex items-start gap-2">
                  <Lightbulb size={14} className="flex-shrink-0 mt-0.5" /><span><strong>전략:</strong> {strategy}</span>
                </div>
              </div>
            ))}
            <div className="mb-12">
              <h3 className="text-2xl font-semibold mb-6">연간 수요 강도 캘린더</h3>
          ...
          <TabsContent value="pricing">
            <h2 className="text-4xl sm:text-5xl font-serif mb-2 border-b pb-4 flex items-center gap-3"><DollarSign size={36} strokeWidth={1.5} />가격 전략</h2>
            <p className="text-slate-500 text-sm tracking-widest uppercase mb-10">기준: 평일 ₩90,000 / 주말 ₩120,000 · 이벤트 배율 적용 · 생성 2026-04-13</p>
            <div className="mb-12">
              <h3 className="text-2xl font-semibold mb-6">이번 달 핵심 액션</h3>
              <div className="space-y-4">
                {[
                  { urgency: '즉시 적용', color: 'border-red-400 bg-red-50', badge: 'bg-red-500 text-white', title: 'DxS ON STAGE (04.17~19) → ₩180,000 + 최소 1박', desc: '인스파이어 아레나 ★★★★ 공연. 4일 후 시작 → 지금 즉시 인상.' },
                  { urgency: '즉시 적용', color: 'border-orange-400 bg-orange-50', badge: 'bg-orange-500 text-white', title: 'T1 HOME GROUND (04.24~26) → ₩180,000 + 최소 1박', desc: 'e스포츠 팬 수요 강함. 3주 전 기준(04.03) 이미 지남 → 지금 즉시 인상.' },
                  { urgency: 'D-14 알림 (04.21)', color: 'border-yellow-400 bg-yellow-50', badge: 'bg-yellow-500 text-white', title: '어린이날 (05.05) → ₩108,000 예정', desc: '공휴일 + 어린이 행사 복합 수요. 04.21에 캘린더 알림 설정.' },
                ].map((item, i) => (
                  <div key={i} className={`border-l-4 rounded-r-xl p-4 ${item.color}`}>
                    <div className="flex items-start gap-3 flex-wrap">
                      <Badge className={`text-xs border-0 ${item.badge}`}>{item.urgency}</Badge>
                      <div>
            ...
            <div className="mb-12">
              <h3 className="text-2xl font-semibold mb-6">4월 날짜별 권장 가격 (잔여: 04.13~30)</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="border-b-2 border-slate-200">
                      ...
                      <th className="text-left py-2 text-slate-400 font-medium uppercase tracking-wider text-xs hidden md:table-cell">타이밍</th>
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      return (
                        <tr key={i} className={`border-b border-slate-100 ${isHighlight ? 'bg-orange-50' : 'hover:bg-slate-50'}`}>
                          ...
                          <td className="py-3 text-slate-500 text-xs hidden md:table-cell">
                            {row.timing === '즉시' ? <span className="flex items-center gap-1"><Zap size={11} className="text-orange-500" />즉시</span> : row.timing}
                          </td>
                        </tr>
                      );
                    })}
            ...
            <div className="mb-12">
              <h3 className="text-2xl font-semibold mb-6">가격 배율 근거</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-200">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-6">
              <h4 className="font-semibold text-xl mb-4 text-blue-900">6월 성수기 준비 데드라인</h4>
              <ul className="space-y-2 text-sm text-blue-800">
                <li className="flex gap-2"><ChevronRight size={14} className="text-blue-400 flex-shrink-0 mt-0.5" /><span><strong>6월 15일 이전:</strong> 7~8월 전체 기간 성수기 가격(₩150,000+) 설정 완료</span></li>
                <li className="flex gap-2"><ChevronRight size={14} className="text-blue-400 flex-shrink-0 mt-0.5" /><span><strong>을왕리 해수욕장 개장 발표 즉시:</strong> 7~8월 ×2.0~3.0 배율 + 최소 2박(주말) 적용</span></li>
                <li className="flex gap-2"><ChevronRight size={14} className="text-blue-400 flex-shrink-0 mt-0.5" /><span><strong>매주 월요일:</strong> 인스파이어 5~6월 공연 라인업 신규 확인</span></li>
              </ul>
            </div>
          </TabsContent>
      ...
      {lightbox && (
        ...
        >
          <Button
            variant="ghost"
            size="icon"
            className="absolute top-6 right-8 text-white hover:text-white hover:bg-white/10 text-2xl w-10 h-10"
            onClick={() => setLightbox(null)}
          >
            <X size={20} />
          </Button>
          ...
          <div className="mt-6 text-center">
            <p className="text-white font-semibold text-xl">{lightbox.title}</p>
