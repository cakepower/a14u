...
### Font Family
*   **Primary**: **DM Sans** (Clean geometric sans-serif)
*   **Secondary/Fallbacks**: **Pretendard**, **Inter**, Helvetica Neue, Arial
*   **Characteristics**: 80px의 히어로 디스플레이부터 12px의 마이크로 레이블까지 깔끔하게 확장되는 가변 서체입니다. 휴머니스트 카운터 덕분에 작은 크기에서도 높은 가독성을 유지합니다.
...
| :--- | :--- | :--- | :--- | :--- |
| **Section Heading** | 600 (SemiBold) | 32px | `{colors.ink}` | `font-semibold text-[32px] leading-[1.25] tracking-[-0.5px] text-zinc-900` |
| **Card Title** | 600 (SemiBold) | 20px | `{colors.ink}` | `font-semibold text-[20px] leading-[1.4] text-zinc-900` |
| **Label** | 500 (Medium) | 14px | `{colors.steel}` | `font-medium text-[14px] leading-[1.5] text-zinc-500` |
| **Value** | 400 (Regular) | 16px | `{colors.charcoal}` | `font-normal text-[16px] leading-[1.5] text-zinc-700` |
| **Score** | 700 (Bold) | 24px | `{colors.brand-blue-deep}` | `font-bold text-[24px] leading-[1.3] text-blue-700` |
...
| :--- | :--- | :--- |
| **Line Height** | **1.4** | 카드 타이틀 (Card Titles) |
| **Line Height** | **1.5 – 1.6** | 본문 및 일반 텍스트 (Body & General Text) |
| **Letter Spacing** | **-0.5px ~ -2px** | 헤드라인 및 디스플레이 텍스트 (시각적 긴장감 부여) |

---

### Weight Discipline
이탤릭체(Italic)를 사용하지 않는 대신, 다음 **4가지 가중치**를 엄격히 구분하여 강조를 표현합니다.

*   **400 (Regular)**: 본문 및 기본 텍스트
*   **500 (Medium)**: 중간 정도의 강조
*   **600 (Semi-Bold)**: 제목 및 버튼 요소
*   **700 (Bold)**: 강한 시각적 강조가 필요한 영역
